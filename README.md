...
```
In this case, instead of "pbs_stdout", "pbs_stderr" is used. The framework will look for the pattern "FDS completed successfully" in the standard error file, fds_33_0.e47843 for instance,   instead of the standard output file, fds_33_0.o47843. And the framework also checks if the file "test-cases/fds/create_remove.out" exists.

Besides pbs_stdout and pbs_stderr, you can put in a file name created by the testing code and a pattern for checking as shown in the following example:
```
                      "columbus": {
                                 "file_patterns": {
...
```
In this case, the framework looks for the pattern "end of argos" in the standard output file and the pattern "driver: overall argos workspace high-water mark =     46415"  in the file "test-cases/columbus/argosls" generated by the testing code.
...
The configuration file is written in json format. It can be named as hpcswtest.json_def and saved in the location indicated by the environmental variable HPCSWTEST_BASE_DIR, or it can be named as hpcswtest.json and saved it in your current working directory. The UTSA revision introduced the keyword "working_dir," which instructs the framework to execute the testing code or command in the specified directory where the test code or test sample data for the application. 
