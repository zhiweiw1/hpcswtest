...
/*checkSubmitResult() is defined in helper file. It is not a class. it checks if the script was submitted sucessfully. It does not check the running result*/
  ...
  for (auto cpp_mpi_src: cpp_mpi_srcs_) {
    cmd_result = compileTest(flog_, getCppJobScripts()[c_i], cpp_mpi_srcs_[c_i], getCppName(), getCppFlags(), getCppSrcName(c_i), getCppExeName(c_i), getCppLinkLibs());
  ...
  for (auto f_mpi_src: f_mpi_srcs_) {
    cmd_result = compileTest(flog_, getFJobScripts()[c_i], f_mpi_srcs_[c_i], getFName(), getFFlags(), getFSrcName(c_i), getFExeName(c_i), getFLinkLibs());
