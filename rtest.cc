...
RTest subclass source code.

Date Created: Tue Dec 9 16:00:00 CST 2021
...
const std::vector<std::string> RTest::r_inputs_ = {
R"(
print "Hello R"
)"
}; // vector r_inputs_
...
std::vector<std::string> RTest::getRModules(void) {
  return python_modules_;
}


R::RTest(const jobscript::JOBSCRIPT &p_s, const std::vector<std::string> &p_ms, const std::string &p_t): AppTest(p_t, ".r", p_s, r_inputs_.size()),
...
*/
void RTest::runRModulesTest(std::ofstream &flog, std::ofstream &fresult, const modules::modules_type &modules, const std::vector<std::string> &python_modules) const {
  std::string result;
  std::string cmd1;
  std::string cmd2;
