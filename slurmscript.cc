...
SlurmScript::SlurmScript(modules::modules_type ms, int t_n_p, int m_n_p_p_n, std::string m_c_n, std::string m_c_a, std::string w_d, std::string e_n, std::string e_a, std::string j_s_n, std::string j_n, std::string q_n, std::string w_t): JobScript(t_n_p, m_n_p_p_n, m_c_n, m_c_a,w_d, e_n, e_a, j_s_n, j_n, q_n, w_t, ms) {}
...
void SlurmScript::generate(void) {
  ...
  if (!getWorkingDir().empty()){
	f_slurm_script << "cd " << getWorkingDir() << std::endl;
  }
  for (auto m_p: m) {
